...
The `SWIM` package contains functions to compare the distribution of model components under different (stressed) models. It is important to note, that functions such as the empirical distribution function, `ecdf()`, or the `quantile()` function applied to a SWIM object will **not** return the empirical distribution function or the quantile function under a stressed model. The empirical distribution functions of model components under a stressed model can be calculated by evaluation of `cdf()` on a SWIM object. Similarly, to calculate sample quantiles of stressed models components, the function `quantile_stressed()` should be used. Implemented visualisation of distribution functions are `plot_cdf()`, for plotting empirical distribution functions, and `plot_hist()`, for plotting histograms of model components under stressed models. The function `VaR_stressed()` and `ES_stressed()` provide the stressed VaR and ES of model components, which is of particular interest for stressed models resulting from constraints on risk measures, see Section \@ref(Sec:RiskMeasures).
