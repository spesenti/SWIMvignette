...
We briefly introduce key concepts, using slightly more technical language compared to Section +++. A *model* consists of a random vector $\mathbf X = (X_1,\dots,X_d)$ and a probability measure; we denote the probability measure of a *baseline model* by $P$ and that of a *stressed model* by $P^W$, where $W= \frac{dP^W}{dP}$, satisfying $E(W)=1,~W\geq 0$ is a Radon-Nikodym derivative. In a Monte-Carlo simulation context, the probability space is discrete with $n$ states $\Omega=\{\omega_1,\dots,\omega_n\}$, each of which corresonds to a simulated scenario. To reconcile this formulation with the notation of Section +++, we denote $X_j(\omega_i):= x_{ji}$ and $W(\omega_i):=\omega_i$. Under the baseline model, each scenario has the same probability $P(\omega_i)=1/n$, while under a stressed model it is $P(\omega_i)=W(\omega_i)/n=w_i/n$.

+++ just started edi  ting, incomplete.
