...
The Wasserstein distance is typically defined via the mass-transportation problem and we refer to @Villani2008book for a detailed treatment of the Wasserstein distance and its properties. For distributions on the real line, the Wasserstein distance admits the above representation which is convenient for numerical evaluations [@Vallender1974TPA]. The difference between the Wasserstein and the Kolmogorov sensitivity is that while the latter determines the largest pointwise distance between a stressed and the baseline model, the former sensitivity sensitivity reflects the entire distribution. 

The Gamma sensitivity is introduced in @Pesenti2019 and we refer to that paper for further properties of the Gamma sensitivity measure and an comparison of the Gamma measure with variance based sensitivity measures as well as moment independent sensitivity measures. The Gamma measure is normalised such that it takes values between -1 and 1, with higher positive (negative) values corresponding to a larger positive (negative) impact of the stress on the particular model component. Loosely speaking, the Gamma sensitivity measure is the normalised difference between the first moment and the stressed and the baseline distribution of a model component. The sensitivity measures can be plotted using `plot_sensitivity`. The function `importance_rank` returns the effective rank of model components according to the chosen sensitivity measure. A small rank of a model component's sensitivity measure corresponds to high sensitivity to that model component. The functions `sensitivity` together with `plot_sensitivity` and `importance_rank` allow for a numerically efficient sensitivity analysis of a model and provides visual assessments of model components' sensitivities to a stressed model. 
