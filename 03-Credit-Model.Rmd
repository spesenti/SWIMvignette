...
Using the function `VaR_stressed` we can quantify how tail quantiles of the aggregate portfolio loss change, when moving from the baseline to the stressed model. We observe that the increase in the VaR of the portfolio loss changes the tail quantiles of the aggregate portfolio loss; thus the stress on VaR induces an increase in ES. The implemented functions `VaR_stressed` and `ES_stressed` calculate respectively VaR and ES at different levels and for different stressed models. The argument `alpha` specifies the levels of VaR and ES, respectively. The stressed model under which the risk measures are calculated can be chosen using `wCol`, which is by default equal to 1. 
