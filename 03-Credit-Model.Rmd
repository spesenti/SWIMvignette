...
In this section, following a reverse sensitivity approach, we study the effects that stresses on (the tail of) the aggregate portfolio $L$ have on the three subportfolios. Through this analysis, we aim to assess the comparative importance of the three subportfolios. First, we impose a $20\%$ increase on the VaR at level 0.9 of the loss of the aggregate portfolio.
...
Using the function `VaR_stressed` we can see how the tail (the quantiles) of the aggregate portfolio changes from the baseline to the stressed model. Further, the increase of the VaR of the aggregate porfolio induces an increase in the ES. The implemented function `ES_stressed` calcuates the ES at different levels (argument `alpha`) and for stressed models specified via the argmuent `wCol` (with default equal to 1)
```{r, CM-stress-VaR-check-ES, cache = FALSE}
VaR_stressed(object = stress.credit, alpha =  seq(0.8, 0.99, by = 0.05), xCol = "L", wCol = 1, base = TRUE)
ES_stressed(object = stress.credit, alpha = 0.9, xCol = "L", base = TRUE)
```


As a second stress, we consider, additionally to the $20\%$ increase in $\text{VaR}_{0.9}$, an increase in $\text{ES}_{0.9}$ of the aggregate portfolio $L$. When stressing VaR and ES together via `stress_VaR_ES`, both VaR and ES need be stressed at the same level, here `alpha = 0.9`. We've seen that stressing the VaR alone, increases ES to `r round(ES_stressed(object = stress.credit, alpha = 0.9, xCol = "L", base = TRUE)[1], 0)`. Thus, for the second stress we want the stressed ES to be equal to 3500. This can be achieved, by specifying the argument `s`, which is the actual stressed value of ES instead of `s_ratio`, the percentage increase. Similarly, one can provide `q_ratio` or `q` for the percentage increase in or actual value of VaR, respectively.
...
```{r, CM-histL, cache = TRUE, fig.cap = "Histogram of the aggregate portfolio loss $L$ under the baseline and the two stressed models."}
...
```{r, CM-plot1, cache = TRUE, fig.cap = "Histogram of the subportfolio $L_2$ for the stress on the VaR (left) and for stressing both VaR and ES (right)."}
...
+++ we should start with talking about the sensitivities of both models. I think we should also look at the Kolmogorov and Wasserstein distance to see that the second stress is "larger", thus further away from the baseline model.+++
...
stress.credit.H2 <- stress_moment(x = stress.credit, f = f.stressH2, k = k.stressH2, m = m.stressH2)
```
...
options(digits = 3)
summary(stress.credit.H2, wCol = 3)
