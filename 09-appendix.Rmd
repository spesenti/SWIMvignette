...
# Appendix Credit Model

This appendix discribes the credit model of Section \@ref(Sec:CreditModel) and provides the code for generating the data. 

## Credit Model assumptions

The credit model is based on the conditionally binomial credit model described in @Mcneil2015B which belongs to the family of mixture models. Specifically, we consider a portfolio that consists of three homogeneous sub-portfolios and denote the total aggregate loss of the portfolio by $L = L_1 + L_2+  L_2$, where $L_1, L_2, L_3$ are the aggregate loss of each sub-portgolio. Each subportfolio's aggregate loss is given by 
\begin{equation}
L_i=e_i\cdot\text{LGD}_i\cdot M_i,\quad i=1,2,3, 
\end{equation}
where $e_i$ and $M_i$ are the exposure and number of insolvencies of the $i^{\text{th}}$ sub-portfolio, respectively. The loss given default of sub-portfolio $i$, $\text{LGD}_i$ is 

**Q** what is the formula for $LGD$?    

The number of insolvencies $M_1, M_2, M_3$ are, conditionally given on $H_1, H_2, H_3$, independent and Binomially distributed with parameters $n_i$ and $H_i$.    

**Q** What is $H_i$?   
**Q** What is $n_i$?   

$H_i$ follows a Beta distribution with shape and rate parameter $p_i$ and $\rho_i$, respectively, where $p_i$ is the default probability and $\rho_i$ the default correlation of sub-portfolio $i$. The dependence structure of $(H_1,H_2,H_3)$ is modelled via a Gaussian copula with correlation matrix +++

**Q** where in the model is $m_i$, the portfolio size? 



The subsequent table sumamrises the parameter values used in the simulation.
...
## Code for generating the data of the credit model
